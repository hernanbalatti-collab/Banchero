...
import type { Pedido, Viaje } from "@/generated/prisma/client";
...
export async function FormViaje({ viaje, pedido }: { viaje?: Viaje; pedido?: Pedido }) {
  // Un viaje nuevo puede arrancar con los datos del pedido de flete del cliente
  const v: Partial<Viaje> | undefined =
    viaje ??
    (pedido && {
      clienteId: pedido.clienteId,
      origen: pedido.origen ?? "",
      destino: pedido.destino ?? "",
      fechaCarga: pedido.fechaCarga ?? undefined,
      descripcionCarga: pedido.descripcion,
      pesoKg: pedido.pesoKg == null ? null : Math.round(pedido.pesoKg),
  ...
  const [depositos, clientes, choferes, vehiculos] = await Promise.all([
    ...
    db.cliente.findMany({
      where: { OR: [{ activo: true }, { id: v?.clienteId ?? undefined }] },
      orderBy: { razonSocial: "asc" },
      select: { id: true, razonSocial: true },
    }),
    db.chofer.findMany({
      where: { OR: [{ activo: true }, { id: v?.choferId ?? undefined }] },
      orderBy: { apellido: "asc" },
      select: { id: true, nombre: true, apellido: true },
    }),
    db.vehiculo.findMany({
      where: { OR: [{ estado: { not: "BAJA" } }, { id: { in: [v?.vehiculoId, v?.acopladoId].filter((x): x is string => !!x) } }] },
      orderBy: { patente: "asc" },
    }),
  ]);
    ...
    <Tarjeta className="max-w-4xl">
      <Form accion={guardarViaje.bind(null, viaje?.id ?? null)} className="space-y-8">
        ...
        <Seccion titulo="Carga y recorrido">
          <Grilla>
            <Entrada name="origen" etiqueta="Origen" valor={v?.origen} placeholder="Ciudad, provincia" ayuda="Entre depósitos se completa solo." />
            <Entrada name="destino" etiqueta="Destino" valor={v?.destino} placeholder="Ciudad, provincia" />
            <Entrada name="fechaCarga" etiqueta="Fecha de carga" type="date" valor={aInputFecha(v?.fechaCarga ?? hoy())} requerido />
            <Entrada name="fechaEntregaEstimada" etiqueta="Entrega estimada" type="date" valor={aInputFecha(v?.fechaEntregaEstimada)} />
            <Entrada
              name="descripcionCarga"
              etiqueta="Descripción de la carga"
              valor={v?.descripcionCarga}
              requerido
          ...
          <Grilla columnas={3}>
            <Entrada name="pesoKg" etiqueta="Peso (kg)" type="number" min={0} valor={v?.pesoKg} />
            <Entrada name="kmEstimados" etiqueta="Km estimados" type="number" min={0} valor={v?.kmEstimados} />
        ...
        <Seccion titulo="Documentación">
          <Grilla>
            <Entrada name="cartaPorte" etiqueta="Carta de porte / CTG" valor={v?.cartaPorte} />
            <Entrada name="remito" etiqueta="Remito" valor={v?.remito} />
            <AreaTexto name="observaciones" etiqueta="Observaciones" valor={v?.observaciones} className="sm:col-span-2" />
          </Grilla>
        </Seccion>

        <div className="flex gap-2 border-t border-stone-100 pt-5">
          <BotonEnviar>{viaje ? "Guardar cambios" : "Crear viaje"}</BotonEnviar>
          <BotonLink href={viaje ? `/viajes/${viaje.id}` : pedido ? `/pedidos/${pedido.id}` : "/viajes"} variante="secundario">
            Cancelar
          </BotonLink>
        </div>
      </Form>
